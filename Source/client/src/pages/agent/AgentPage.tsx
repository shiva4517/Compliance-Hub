...
export default function AgentPage() {
  ...
  // Prevent useEffect from overwriting messages while a send is in flight
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      // Stabilize optimistic user bubble (keep it visible) then append assistant reply
      setMessages((prev) => {
      ...
      setIsTyping(false);
      setBanner(null);
      // Invalidate conversation list (title may have changed)
      qc.invalidateQueries({ queryKey: ['agent-conversations'] });
      // Invalidate companies cache if agent created/updated/deleted a company
      const companyOps = ['create_company', 'update_company', 'delete_company'];
      if (res.lastOperation && companyOps.includes(res.lastOperation)) {
