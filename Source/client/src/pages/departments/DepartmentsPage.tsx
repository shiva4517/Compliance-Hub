...
import { Plus, Search, Pencil, Layers, ToggleLeft, ToggleRight } from 'lucide-react';
...
export default function DepartmentsPage() {
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: unknown) => {
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message;
      setUpdateError(msg ?? 'Failed to update department.');
    }
  });

  const toggleMutation = useMutation({
    mutationFn: ({ id, isActive }: { id: string; isActive: boolean }) =>
      departmentService.toggleStatus(id, isActive),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['departments', companyId] });
    ...
    <div>
      <div className="flex items-center justify-between mb-1">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Define and manage your company's department structure.</p>
        </div>
      ...
      <div className="mt-4 mb-4 flex items-center gap-3">
        ...
        {!isLoading && (
          <span className="text-sm text-gray-500">
            Showing {filtered.length} of {data?.items.length ?? 0} departments
          </span>
        )}
      </div>
        ...
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="table-th">Divisions</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {isLoading ? (
              ...
              <tr key={d.id} className="hover:bg-gray-50">
                ...
                <td className="table-td text-gray-600">{d.divisionCount}</td>
                ...
                <td className="table-td">
                  <div className="flex items-center gap-1">
                    <button onClick={() => { setUpdateError(''); setEditDept(d); }}
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)}
        title="Add Department" subtitle="Create a new department for your company." size="md">
        <DepartmentForm
      ...
      <Modal isOpen={!!editDept} onClose={() => setEditDept(null)}
        title="Edit Department" size="md">
        {editDept && (
          <DepartmentForm
            defaultValues={editDept}
            submitLabel="Save Changes"
            onSubmit={d => updateMutation.mutate({ id: editDept.id, ...d })}
      ...
      <ConfirmDialog
        ...
        message={toggleTarget?.isActive
          ? `Deactivate "${toggleTarget.name}"? Its divisions will remain but may be affected.`
